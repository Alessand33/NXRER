...
% 19 Phs 
exp((1-exp(thf))*(Phs+q)) = exp(0*xif)^(1-exp(thf))*(exp(thf)*exp(Wh-Zh)/(exp(thf)-1))^(1-exp(thf))*exp(PsiXh);
% 20 Pf 
exp((1-exp(thh))*(Pf-q)) = exp(0*xih)^(1-exp(thh))*(exp(thh)*exp(Wf-Zf)/(exp(thh)-1))^(1-exp(thh))*exp(PsiXf);
% 21 PsiXh
...
% 37 Px 
exp(Px) = exp(Phs+q-0*xif+1/(exp(thf)-1)*Nh) + sdtot*etot/2;
% 38 Pm
exp(Pm) = exp(Pf-0*xih+1/(exp(thh)-1)*Nf) - sdtot*etot/2;
% 39 EXR
exp(EXR) = exp(EXN-Px);
% 40 IMR
exp(IMR) = exp(IMN-Pm);
...
estimation(datafile=Data02_world, mode_compute = 6, nobs=139,mh_replic=20000,mh_nblocks=5,mh_jscale=0.3, plot_priors = 1, nodisplay);
